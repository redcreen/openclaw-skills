...
1. 安装 `health` 能力
2. 告诉它你的基本情况、目标和背景，完成建档
3. 日常发体重、血压、运动、症状、用药相关图片或事实
...
还没完全补齐，但 roadmap 已经锁定的能力：

- 一键安装整套 `health` skill 集
- 更完整的主动建档和三高初评
- 日 / 周 / 月复盘
- 医生速读摘要
- 提醒机制
...
1. 先告诉它你的年龄、身高、当前目标、三高相关背景、当前用药
2. 或者直接发第一张体重 / 血压 / 运动图片
3. 让它帮你“记录并告诉我这意味着什么”

例如：

```text
这是我今天早上的体重图，帮我记一下，并看看最近趋势
这是我今天的血压图，记录后告诉我今天要注意什么
...
- 至少装 `health-archive + private-doctor`

roadmap 的目标形态不是把 `health/` 伪装成一个大 skill，而是提供“一条命令安装整套 `health` skill 集”的入口。
...
- roadmap 的目标不是把 `health/` 伪装成一个大 skill
- 目标是提供“一条命令安装多个 health skill”的整套安装入口
- 默认整套能力至少包含：
  - `health-archive`
  - `private-doctor`
  - `health-review`
