[English](README.md) | [中文](README.zh-CN.md)

# 文档首页

这组文档用于说明这个工作区怎么组织、怎么新增 skill 集，以及怎么安装和验证单个 skill。

## 从这里开始

- 工作区总览: [../README.zh-CN.md](../README.zh-CN.md)
- English overview: [../README.md](../README.md)
- `health` skill 集: [../health/README.zh-CN.md](../health/README.zh-CN.md)

## 架构

- 工作区架构: [architecture.zh-CN.md](architecture.zh-CN.md)
- English architecture: [architecture.md](architecture.md)

## 规划

- 路线图: [roadmap.zh-CN.md](roadmap.zh-CN.md)
- English roadmap: [roadmap.md](roadmap.md)
- 开发计划: [reference/openclaw-skills/development-plan.zh-CN.md](reference/openclaw-skills/development-plan.zh-CN.md)
- Health 能力映射: [reference/openclaw-skills/health-capability-map.zh-CN.md](reference/openclaw-skills/health-capability-map.zh-CN.md)
- Health 重置 Playbook: [reference/openclaw-skills/health-reset-playbook.zh-CN.md](reference/openclaw-skills/health-reset-playbook.zh-CN.md)

## 验证

- 测试计划: [test-plan.zh-CN.md](test-plan.zh-CN.md)
- English test plan: [test-plan.md](test-plan.md)

## 参考资料

- 参考资料首页: [reference/README.zh-CN.md](reference/README.zh-CN.md)
- GitHub 安装指引: [reference/openclaw-skills/github-install.zh-CN.md](reference/openclaw-skills/github-install.zh-CN.md)
- 工作流资料: [workstreams/README.zh-CN.md](workstreams/README.zh-CN.md)
- 开发日志: [devlog/README.zh-CN.md](devlog/README.zh-CN.md)
- 归档文档: [archive/README.zh-CN.md](archive/README.zh-CN.md)
