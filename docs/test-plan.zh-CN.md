...
验证工作区结构、文档路由，以及隔离 skill 集所需的最小可安装形态。
...
- Case: 每个 health skill 都可独立安装
  - Setup: 查看 `health/health-archive/` 和 `health/private-doctor/`
  - Action: 检查目录内容
...
- Case: GitHub 直装入口清晰可复制
  - Setup: 打开根 README 和单 skill README
  - Action: 检查 GitHub 直装示例
  - Expected Result: 用户能直接复制单个 skill 的 GitHub tree 地址到 OpenClaw 对话框；文档优先推荐 tag 地址
