[English](README.md) | [中文](README.zh-CN.md)

# Documentation

This documentation set explains how the workspace is organized, how new skill sets are added, and how individual skills are installed and validated.

## Getting Started

- Workspace overview: [../README.md](../README.md)
- Chinese overview: [../README.zh-CN.md](../README.zh-CN.md)
- `health` skill set: [../health/README.md](../health/README.md)

## Architecture

- Workspace architecture: [architecture.md](architecture.md)
- Chinese architecture: [architecture.zh-CN.md](architecture.zh-CN.md)

## Planning

- Roadmap: [roadmap.md](roadmap.md)
- Chinese roadmap: [roadmap.zh-CN.md](roadmap.zh-CN.md)
- Development plan: [reference/openclaw-skills/development-plan.md](reference/openclaw-skills/development-plan.md)
- Health capability map: [reference/openclaw-skills/health-capability-map.md](reference/openclaw-skills/health-capability-map.md)
 - Health reset playbook: [reference/openclaw-skills/health-reset-playbook.md](reference/openclaw-skills/health-reset-playbook.md)

## Verification

- Test plan: [test-plan.md](test-plan.md)
- Chinese test plan: [test-plan.zh-CN.md](test-plan.zh-CN.md)

## Reference

- Reference index: [reference/README.md](reference/README.md)
- GitHub install guide: [reference/openclaw-skills/github-install.md](reference/openclaw-skills/github-install.md)
- Workstreams: [workstreams/README.md](workstreams/README.md)
- Devlog: [devlog/README.md](devlog/README.md)
- Archive: [archive/README.md](archive/README.md)
